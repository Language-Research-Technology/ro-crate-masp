...
class PropertyRule extends Rule {
  ...
  validateFixedValue(entity, propertyValues) {
    ...
    if (!propertyValues && this.minCount > 0) {
      this.lastFailureReason = `missing required property ${this.propertyName} with fixed value ${expectedValue}`;
      this.log(
    ...
    if (propertyValues) {
      ...
      if (!matches) {
        this.lastFailureReason = `values do not match expected fixed value ${expectedValue}`;
        this.log(
  ...
  validatePropertyConstraints(entity, propertyValues) {
    ...
    for (const value of propertyValues) {
      ...
      const failReason = this.validator.validatePropertyValue(value, this.rangeIncludes);
        if (failReason !== null) {
...
class MaspValidator {
  ...
  parseRules() {
    ...
    if (this.rules.__rootRuleId) {
      ...
      for (const propertyRule of Object.values(this.rules.properties)) {
        ...
        const isRootIdProperty =
          ...
          Array.isArray(propertyRule.fixedValue) &&
          propertyRule.fixedValue.includes("ro-crate-metadata.json");
    ...
    for (let entity of this.profileCrate.root.hasResource || []) {
      if (
        ...
        for (const valueEntity of entity["hasPart"] || []) {
  ...
  getConformsToUris() {
    // editorHints.conformsToUri takes precedence for backward compatibility
    if (Array.isArray(this.editorHints.conformsToUri) && this.editorHints.conformsToUri.length > 0) {
      return this.editorHints.conformsToUri;
    }
    // Fall back to deriving from the profile crate root entity @id
    ...
    if (this.profileCrate) {
      ...
      if (rootId) {
        ...
        if (this.profileBaseUrl) {
          try {
            return new URL(rootId, this.profileBaseUrl).toString();
          } catch (_) {
            // fall through
          }
        }
  ...
  getProfileEntity() {
    ...
    const profileUri = this.getProfileUri();
    return {
      "@id": profileUri || root["@id"],
      "@type": root["@type"] || ["Profile"],
      name: root.name,
      url: root.url,
    };
  }
  ...
  toEditorDefinition(propertyRule) {
    ...
    return {
      ...
      type: propertyRule.fixedValue ? ["Value"] : (types.length ? types : ["Text"]),
  ...
  getEditorValuesForProperty(propertyRule) {
    if (propertyRule.fixedValue) {
      return asArray(propertyRule.fixedValue);
    }
