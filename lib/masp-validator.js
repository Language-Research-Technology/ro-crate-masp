...
// Yields to the host environment so a pending DOM update actually gets
// painted before the next chunk of synchronous work starts. setTimeout(0)
// does NOT reliably do this on its own — it queues a macrotask, but a
// browser can (and often does) run several back-to-back without ever
// inserting a render step, especially when each one finishes quickly.
// requestAnimationFrame is the primitive that specifically waits for the
// next paint, but it has the opposite failure mode: browsers throttle or
// fully suspend rAF callbacks whenever the page isn't actually visible
// (a backgrounded tab, an occluded/embedded view) — waiting on it alone
// can stall indefinitely in exactly the case where nobody's watching the
// bar anyway. Racing both gets a real paint opportunity when visible and a
// hard ceiling on wait time when not; setTimeout(0) alone covers
// environments with no rAF at all (Node/CLI usage, tests).
...
class MaspValidator {
  constructor(profileCratePathOrObject, options = {}) {
    ...
    // Optional progress callback, invoked once per class rule as
    // validateCrate works through them: onProgress({ phase, current, total,
    // ratio, classRule }). Can also be passed per-call to validateCrate().
   ...
   */
  async validateCrate(targetCrate, options = {}) {
    const onProgress = typeof options.onProgress === "function" ? options.onProgress : this.onProgress;
    ...
    try {
      ...
      await this.validateTargetCrateGraph(crate, onProgress);
  ...
  /**
   * Validate the target crate graph against the SoSS+ profile using Rule
   * objects. Reports progress once per class rule (not per entity) so
   * ClassRule.validate() keeps its existing synchronous, non-Promise
   * contract — nothing else in this codebase calls it directly, but this
   * keeps the change safe for any external subclass/monkeypatch too. Each
   * step also yields to the event loop so a caller-supplied onProgress can
   * actually get painted (e.g. a browser progress bar) instead of the whole
   * validation running as one uninterrupted synchronous block.
   */
  async validateTargetCrateGraph(targetCrate, onProgress) {
    const classRules = Object.values(this.rules.classes);
    const total = classRules.length;
    let current = 0;
      ...
      if (onProgress) {
        try {
          onProgress({
            ...
            current,
            total,
            ratio: total ? current / total : 1,
            classRule: classRule.name,
          });
        } catch (error) {
          // A misbehaving progress callback must not break validation itself.
          this.log(`onProgress callback threw: ${error.message}`, "warn");
        }
      }

      // Yield between class rules so the host UI can repaint.
      await yieldToHost();
    }
  }
